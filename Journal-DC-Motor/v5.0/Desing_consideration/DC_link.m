...
fourier_all=fourier_tx(:,2:5)+fourier_motor(:,1:4);
%%
figure1 = figure;
...
ylabel({'$I_{capM}$ ($A_{RMS}$)'},'FontSize',10,...
...
for k=1:1:4
hold on;
plot(Dx,abs(fourier_motor(:,k))/sqrt(2),'LineWidth',2,'Color',C(k,:))
end
ylabel({'$I_{CAP}$ ($A_{RMS}$)'},'FontSize',11,...
    'FontName','Times New Roman',...
    'Interpreter','latex');
...
xlabel('Duty Cycle','FontSize',11,'FontName','Times New Roman',...
    'Interpreter','latex');
set(gca,'FontName','Times New Roman','FontSize',11);
legend1=legend('$1^{st}$','$2^{nd}$','$3^{rd}$','$4^{th}$','Location','Best');
set(legend1,...
    'Location','best',...
    'Interpreter','latex',...
    'FontSize',10,...
...
for k=1:1:4
hold on;
plot(Dx,abs(fourier_all(:,k))/sqrt(2),'LineWidth',2,'Color',C(k,:))
end
ylabel({'$I_{CAP}$ ($A_{RMS}$)'},'FontSize',11,...
    'FontName','Times New Roman',...
    'Interpreter','latex');
