...
S=zeros(size(S1));
for i=1:length(S1)
    if (abs(S1(i)-S3(i))<2*S2(i)) && (S1(i)+S3(i)> 2*S2(i))
    S(i)=0;
    elseif abs(S1(i)-S3(i))> 2*S2(i)
%     S(i)=S1(i)-S2(i)-S3(i)-S2(i);   
%      S(i)= abs(S1(i)+S3(i))-abs(2*S2(i));
     S(i)= abs(S1(i)-S3(i))-2*S2(i);
   
    elseif (S1(i)+S3(i)< 2*S2(i))    
%        S(i)= abs(S1(i)-S3(i))-abs(2*S2(i));
        S(i)= -abs(S1(i)+S3(i))+2*S2(i);
%         S(i)=0;
    end
...
plot(theta_f1*180/pi,S,'b')
hold on;
% plot(theta_f1*180/pi,-abs(S1+S2)+abs(S3-S2),'b')
hold on;
plot(theta_f1*180/pi,abs(S1+2*S2+S3),'r')
